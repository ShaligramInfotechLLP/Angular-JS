app.controller('ProductDetailController', function (StoreSessionDetail, UserDetail, $timeout, $state, $http, $rootScope, $stateParams, $filter, $scope, $window, $state, notificationFactory, configurationService, $compile, $interval, $http, $log, $q) {
    
    decodeParams($stateParams);
    BindToolTip();
    Tab();
    $scope.StoreDetailInSession = StoreSessionDetail;
    $scope.CustomerId = $scope.CustomerDetail != null && $scope.CustomerDetail.customer_id > 0 ? $scope.CustomerDetail.customer_id : 0;

    
    if (!$stateParams.ProductId && !$stateParams.ProductId) {
        $scope.SelectedProductId = parseInt($stateParams.ProductId);
    }

    $scope.GetProductDetail = function () {
        $http.get(configurationService.basePath + "API/ProductApi/GetProductDetail?StoreID=" + $scope.StoreDetailInSession.store_id + "&ProductId=" + $scope.SelectedProductId + "&CustomerId=" + $scope.CustomerId)
